...
const data = [[3, 2], [5, 5], [1, 1], [4, 3], [5, 1], [2, 1], [5, 3], [2, 2], [3, 5], [1, 3], [3, 3]];
...
const centroids = [[2, 2], [3, 3], [5, 3], [3, 5]];
/*
...
function kmeans(data, centroids, k) {
    ...
    do { // 2. Toistetaan kohtia 3-4
        ...
        for (let i = 0; i < data.length; i++) { // käydään läpi luokiteltava data
            ...
            for (let cluster = 0; cluster < k; cluster++) { // käydään läpi k kpl klustereita
                let dist = 0; // alustetaan keskipisteen ja datapisteen etäisyys nollaksi
                for (let j = 0; j < data[i].length; j++) { // käydään läpi datapiste, tässä 2 numeroa
                ...
                if (smallestdistance === -1) { // ensimmäinen kierros
                ...
                } else if (tempdistance <= smallestdistance) { // seuraavat kierrokset
        ...
        for (let cluster = 0; cluster < k; cluster++) { // käydään läpi k kpl klustereita
            ...
            for (let i = 0, clustergroup = Groups[cluster].length; i < clustergroup; i++) {
                // käydään läpi taulukoiden sisältämät datapisteet
                for (let j = 0, clustergroupsize = Groups[cluster][i].length; j < clustergroupsize; j++) {
                    // lisätään centroideihin Groups -taulukon alkiot
                    centroids[cluster][j] += Groups[cluster][i][j];
                    // console.log(centroids[cluster][j]);
                }
            }
            for (let i = 0; i < centroids[cluster].length; i++) { // käydään läpi centroidit
                ...
                if (centroids[cluster][i] != oldcentroids[cluster][i]) { // jos centroidit ovat muuttuneet
                    changed = true;
...
console.log('Lopulliset klusterit järjestyksessä vihervasemmisto, keskusta, perussuomalaiset, kokoomus: ');
console.log(groups);